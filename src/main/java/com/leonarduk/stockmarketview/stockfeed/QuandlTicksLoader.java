...
public class QuandlTicksLoader {
	...
	public static class QuandlDataFeed {
		...
		private URL buildURL() {
			String urlString = String.format(
					"https://www.quandl.com/api/v3/datasets/%s.csv?start_date=%s&end_date=%s&order=asc", quandlCode,
					DATE_FORMATTER.print(startDate), DATE_FORMATTER.print(endDate));
		...
		private TimeSeries toTimeSeries(InputStream is) {
			...
			try {
				...
				Logger.getLogger(QuandlTicksLoader.class.getName()).log(Level.SEVERE, "Unable to load ticks from CSV",
						ioe);
	...
	public static void main(String args[]) {
		QuandlDataFeed quandl = new QuandlDataFeed("YAHOO/PHGP.L", DateTime.now().minusYears(20));
