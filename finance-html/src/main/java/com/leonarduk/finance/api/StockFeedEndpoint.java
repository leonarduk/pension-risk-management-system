...
//	}
	...
	public Response downloadHistoryCsv(@PathParam("ticker") final String ticker,
	        ...
	        throws IOException {
		final Instrument instrument = Instrument.fromString(ticker);
		final List<ExtendedHistoricalQuote> series = this.getHistoryData(instrument,
		        years == 0 ? 1 : years, interpolate);
	...
	public List<ExtendedHistoricalQuote> getHistory(
	...
	private List<ExtendedHistoricalQuote> getHistoryData(final Instrument instrument,
	        final int years, final boolean interpolate) throws IOException {
		final Optional<StockV1> stock = this.stockFeed.get(instrument, years,
		        interpolate);
		if (stock.isPresent()) {
