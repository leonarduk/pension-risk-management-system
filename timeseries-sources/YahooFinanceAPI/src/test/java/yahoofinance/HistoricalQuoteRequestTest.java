...
public class HistoricalQuoteRequestTest extends MockedServersTest {
    ...
    public void historicalQuoteTest() throws IOException {
        ...
        for (HistoricalQuote histQuote : goog.getHistory()) {
            ...
            assertTrue(histQuote.getVolumeAsLong() > 0);
            assertNotNull(histQuote.getDateAsCalendar());
        }
        ...
        assertEquals(Long.valueOf(2125700), histQuote.getVolumeAsLong());
        assertEquals(3, histQuote.getDateAsCalendar().get(Calendar.MONTH));
        assertEquals(1, histQuote.getDateAsCalendar().get(Calendar.DATE));
        assertEquals(2016, histQuote.getDateAsCalendar().get(Calendar.YEAR));
    ...
    public void multiYearTest() throws IOException {
        ...
        assertEquals(8, histQuote.getDateAsCalendar().get(Calendar.MONTH));
        assertEquals(6, histQuote.getDateAsCalendar().get(Calendar.DATE));
        assertEquals(2016, histQuote.getDateAsCalendar().get(Calendar.YEAR));

        histQuote = goog.getHistory().get(260);
        assertEquals(8, histQuote.getDateAsCalendar().get(Calendar.MONTH));
        assertEquals(12, histQuote.getDateAsCalendar().get(Calendar.DATE));
        assertEquals(2011, histQuote.getDateAsCalendar().get(Calendar.YEAR));
