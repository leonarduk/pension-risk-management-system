...
def apply_technical_indicators(
    df: pd.DataFrame, price_col: str = "Price"
) -> pd.DataFrame:
    df = df.copy()
...
def generate_signals(
    df: pd.DataFrame, group_by_date: bool = False, lookahead_days: int = 3
) -> pd.DataFrame:
    signals = []
        ...
        if current_gap < 0 and future_gap > 0:
            signals.append(
                (
                    df.index[-1],
                    df.iloc[-1]["Price"],
                    f"🔮 MACD Bullish Crossover Likely in {lookahead_days}d",
                )
            )
            ...
            signals.append(
                (
                    df.index[-1],
                    df.iloc[-1]["Price"],
                    f"🔮 MACD Bearish Crossover Likely in {lookahead_days}d",
                )
            )
    ...
    if group_by_date and not df_signals.empty:
        df_signals = (
            df_signals.groupby("Date")
            .agg({"Price": "first", "Signal": lambda x: "; ".join(sorted(x))})
            .reset_index()
        )
...
def colorize(signal):
    ...
    if (
        "ideal buy" in signal_lower
        or "bullish" in signal_lower
        or "oversold" in signal_lower
        or "crossed above" in signal_lower
    ):
        return f"\033[92m{signal}\033[0m"
    elif (
        "sell" in signal_lower
        or "bearish" in signal_lower
        or "overbought" in signal_lower
        or "crossed below" in signal_lower
    ):
...
def analyze_all_tickers(
    ...
    for ticker in tickers:
        ...
        annual_yield, div_series = last_12m_dividend_yield(
            ticker=ticker,
            timeseries=None,
            use_stockfeed=False,
            xml_path=xml_path,
        ...
        plot_technical_indicators(
            df, ticker=ticker, save_path=plot_path, signals_df=signals_df
        )
        ...
        if not signals_df.empty:
            ...
            if "likely" in latest["Signal"].lower():
                ...
                "sell" in latest["Signal"].lower()
                or "bearish" in latest["Signal"].lower()
                or "overbought" in latest["Signal"].lower()
            ):
...
def get_price_series(
    ...
    *,
    ...
    # ▸ Case B: fall back to whatever other shapes you support
        ...
        price = timeseries.rename(columns=str.lower).set_index("date")["close"]
...
def last_12m_dividend_yield(
    ...
    *,
    ...
    if annual_yield > 50:
        print(
            f"⚠️  {ticker} has an unusually high yield of {annual_yield:.2f}% so dividing by 100."
        )
        annual_yield /= 100
