...
public class LinearInterpolator extends AbstractLineInterpolator {
	...
	@Override
	protected HistoricalQuote calculatePastValue(final HistoricalQuote firstQuote,
	...
	public HistoricalQuote createSyntheticQuote(final HistoricalQuote currentQuote,
		...
		final double timeInteval = DateUtils.getDiffInWorkDays(nextQuote.getDate(),
		        currentQuote.getDate());
		final int dayCount = DateUtils.getDiffInWorkDays(currentQuote.getDate(), currentDate);
		...
		return this.createSyntheticQuote(currentQuote, currentDate, newClosePrice, newOpenPrice,
		        "Interpolated from " + currentQuote.getDate() + "(" + currentQuote.getClose()
		                + ") to " + nextQuote.getDate() + " (" + nextQuote.getClose() + ")");
	}
