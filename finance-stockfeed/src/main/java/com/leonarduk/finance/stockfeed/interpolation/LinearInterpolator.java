...
public class LinearInterpolator extends AbstractLineInterpolator {
	...
	@Override
	protected HistoricalQuote calculatePastValue(final HistoricalQuote firstQuote, final LocalDate fromDate) {
		// TODO maybe use a gradient from a few points before
		throw new UnsupportedOperationException();
	}
	...
	public HistoricalQuote createSyntheticQuote(final HistoricalQuote currentQuote, final LocalDate currentDate,
		...
		final double timeInteval = DateUtils.getDiffInWorkDays(nextQuote.getDate(), currentQuote.getDate());
