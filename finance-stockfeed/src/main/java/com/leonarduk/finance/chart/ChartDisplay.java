...
public class ChartDisplay {
	public static void displayChartInFrame(final JFreeChart chart,
	...
	public static StringBuilder getTable(final Map<?, ?> map, final String col1,
		...
		final StringBuilder sb = new StringBuilder("<table><tr><th>")
		        .append(col1).append("</th><th>").append(col2)
		        .append("</th></tr>");
		map.entrySet().stream().forEach(entry -> sb.append("<tr><td>")
		        .append(entry.getKey().toString()).append("</td><td>")
		        .append(entry.getValue().toString()).append("</td></tr>"));
		sb.append("</table>");
		return sb;
	}

	public static void saveChartAsPng(final String imageFileName,
	        final JFreeChart chart, final int width, final int height)
	        throws IOException {
		final ChartRenderingInfo info = new ChartRenderingInfo(
		        new StandardEntityCollection());

		ChartUtilities.saveChartAsPNG(new File(imageFileName), chart, width,
		        height, info);

	}

	public static StringBuilder saveImageAsPngAndReturnHtmlLink(
	        final String imageNamePrefix, final int width, final int height,
		...
		final StringBuilder sb = new StringBuilder("<IMG SRC=\"")
		        .append(imageName).append("\" WIDTH=\"").append(width)
		        .append("\" HEIGHT=\"").append(height)
		        .append("\" BORDER=\"0\" USEMAP=\"#chart\"/>");
		return sb;
	}

	public static StringBuilder saveImageAsSvgAndReturnHtmlLink(
	        final String imageNamePrefix, final int width, final int height,
	        final JFreeChart chart) throws IOException {
